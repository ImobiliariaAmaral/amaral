/* =========================================================
   ...
   - LocalStorage + migração automática
   ========================================================= */
...
(() => {
  ...
  // Links da vitrine (configuráveis)
  const SOCIAL = {
  ...
  // Placeholder caso não tenha foto (não quebra vitrine/adm)
  ...
  function atualizarImovel(id, obj) {
    const lista = carregarImoveis();
    const idx = lista.findIndex((x) => x.id === id);
    if (idx === -1) return lista;

    lista[idx] = { ...lista[idx], ...obj, id };
    salvarImoveis(lista);
    return lista;
  }

  function excluirImovel(id) {
    const lista = carregarImoveis().filter((x) => x.id !== id);
    salvarImoveis(lista);
    return lista;
  }
 ...
 function initADM() {
  ...
  function refreshDashboard(lista) {
    if (!dashTotal) return; // se não tiver no HTML, não quebra
  ...
  /* -----------------------------
     MODAL FORM: abrir/fechar
  ----------------------------- */
  function openFormModal(mode, imv = null) {
    // Se você ainda não colocou o modal no HTML, evita quebrar:
    if (!formOverlay) {
      // fallback: só rola pro topo do form
  ...
  function renderTable(lista) {
    ...
    lista.forEach((imv) => {
      ...
      // Foto | Título | Bairro/Cidade | Tipo | Preço | m² | Status | Ações
      tr.innerHTML = `
  ...
  tableBody.addEventListener("click", (e) => {
    ...
    // Se clicou em ação, não abre drawer
  ...
  return items
    .filter(([, v]) => Number(v) > 0) // se quiser mostrar 0 também, remove essa linha
    .map(([k, v, icon]) => `
      <div class="info-card icon-card">
        <div class="ic">${typeof icon === "string" ? icon : icon}</div>
  ...
  <div class="prop-modal">
    ...
    <!-- ESTRUTURA COM ÍCONES (cards bonitos) -->
...
`;
  ...
  /* Expondo funções (debug) */
  window.carregarImoveis = carregarImoveis;
  window.salvarImoveis = salvarImoveis;
